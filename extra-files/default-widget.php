	...
	function widget( $args, $instance ) {
		...
		if ( $comments ) {
			foreach ( (array) $comments as $comment) {
				$output .=  '<li class="recentcomments">' . /* translators: comments widget: 1: comment author, 2: post link */ sprintf(_x('%1$s  %2$s', 'widgets'), get_comment_author_link(), '「<a class="comment_content" href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . strip_tags($comment->comment_content) . '</a>」') . '</li>';
			}
 		}
		$output .= '</ul>';
		$output .= $after_widget;
